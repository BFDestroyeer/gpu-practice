...
void daxpy(int n, double a, float *x, int incx, double *y, int incy)
{
    for (int i = 0; i < n; i++)
    {
        y[i * incy] = y[i * incy] + a * x[i * incx];
...
void daxpyOpenMP(int n, double a, float *x, int incx, double *y, int incy)
{
#pragma omp parallel for default(none) shared(n, y, incy, a, x, incx)
    for (int i = 0; i < n; i++)
    {
...
void saxpyOpenCL(int n, float a, float *x, int incx, float *y, int incy, cl_device_id deviceId, double *computationTime)
{
    ...
    clSetKernelArg(kernel, 4, sizeof(cl_mem), &xBuffer);
...
void daxpyOpenCL(int n, double a, float *x, int incx, double *y, int incy, cl_device_id deviceId,
